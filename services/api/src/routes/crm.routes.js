...
router.get('/contacts', async (req, res) => {
  ...
  try {
    ...
    let query = `
      SELECT
        c.*,
        mc.name as st_customer_name,
        mc.email as st_email,
        mc.phone as st_phone
      FROM ${SCHEMA.crm}.crm_contacts c
      LEFT JOIN ${SCHEMA.master}.customers mc ON c.st_customer_id = mc.st_id
    `;
