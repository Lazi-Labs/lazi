...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Try cache first
    const cacheKeyStr = cacheKey('organization', tenantId, 'health');
    ...
    await setCache(cacheKeyStr, response, CACHE_TTL.categories || 300);
    res.json(response);
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const buildIssueConditions = (prefix, tableType) => {
      const conditions = [];
      const imageColumn = tableType === 'materials' ? 's3_image_url' : 's3_image_url';
      const categoryColumn = tableType === 'materials' ? 'categories' : 'category_st_id';

      if (!issueType || issueType === 'uncategorized') {
        if (tableType === 'materials') {
          conditions.push(`(${prefix}.categories IS NULL OR ${prefix}.categories = '[]'::jsonb)`);
        } else {
          conditions.push(`(${prefix}.category_st_id IS NULL OR ${prefix}.category_st_id = 0)`);
        }
      }
      if (!issueType || issueType === 'no_image') {
        conditions.push(`(${prefix}.s3_image_url IS NULL OR ${prefix}.s3_image_url = '')`);
      }
    ...
    if (entityType === 'all' || entityType === 'materials') {
      queries.push(`
        SELECT
          ...
          m.price,
          m.cost,
          m.s3_image_url as image_url,
          NULL::bigint as category_st_id,
          NULL as category_name,
          m.is_reviewed,
          m.health_score,
          ARRAY_REMOVE(ARRAY[
            CASE WHEN m.categories IS NULL OR m.categories = '[]'::jsonb THEN 'uncategorized' END,
            CASE WHEN m.s3_image_url IS NULL OR m.s3_image_url = '' THEN 'no_image' END,
        ...
        WHERE m.tenant_id = '${tenantId}' AND m.active = true AND (${buildIssueConditions('m', 'materials')})
      `);
    }
      ...
      queries.push(`
        SELECT
          ...
          ARRAY_REMOVE(ARRAY[
            ...
            CASE WHEN s.s3_image_url IS NULL OR s.s3_image_url = '' THEN 'no_image' END,
        ...
        WHERE s.tenant_id = '${tenantId}' AND s.active = true AND (${buildIssueConditions('s', 'services')})
      `);
    }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existingGroups.rows.length > 0) {
      ...
      const imageColumn = 's3_image_url';

      const groupsWithDetails = await Promise.all(
        existingGroups.rows.map(async (group) => {
          const items = await pool.query(`
            SELECT st_id, code, name, price, ${imageColumn} as image_url
            FROM ${table}
  ...
  asyncHandler(async (req, res) => {
    ...
    const completion = await pool.query(`
      SELECT
        ...
        COUNT(*) FILTER (WHERE categories IS NOT NULL AND categories != '[]'::jsonb) as categorized,
        COUNT(*) FILTER (WHERE s3_image_url IS NOT NULL AND s3_image_url != '') as imaged,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await invalidateCache(`pricebook:${tenantId}:organization:*`);
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      res.json({
        ...
        counts: {},
        total: 0,
        note: 'Pending sync view not available',
      });
    }
  })
);
