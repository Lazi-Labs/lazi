...
export async function resolveImageUrl(entityType, stId, tenantId) {
  ...
  try {
    ...
    // Simple query without CRM table join (CRM tables may not exist yet)
    const query = `
      SELECT 
        m.st_id,
    ...
    // Priority: S3 migrated → ST original
    return row.migrated_url || row.original_url || null;
  } finally {
    await pool.end();
  }
}
...
export async function resolveImageUrls(entityType, stIds, tenantId) {
  ...
  try {
    ...
    for (const row of result.rows) {
      // Priority: S3 migrated → ST original
      imageMap[row.st_id] = row.migrated_url || row.original_url || null;
    }
