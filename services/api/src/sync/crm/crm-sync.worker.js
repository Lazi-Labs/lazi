/**
 * CRM Sync Worker
 * Syncs ServiceTitan data to Perfect Catch CRM
 *
 * Sync Steps:
    ...
    for (const estimate of estimatesResult.rows) {
      try {
        ...
        if (estimate.existing_opp_id) {
          ...
          // Only move forward in pipeline - check stage order
          // Stage order: contacted(2) < appointment-scheduled(3) < proposal-sent(4)
          const stageOrder = {
            'new-lead': 1,
            'contacted': 2,
          ...
          const currentOrder = stageOrder[estimate.existing_stage] || 0;
          const proposalOrder = stageOrder['proposal-sent'];
          
          // Only update stage if moving forward (current stage is before proposal-sent)
          const updateData = {
            title,
          ...
          // Only move to Proposal Sent if currently in an earlier stage
          if (currentOrder < proposalOrder) {
            updateData.stage = SALES.stages.PROPOSAL_SENT.id;
          }

          // Update existing opportunity
          await updateOpportunity(estimate.existing_opp_id, updateData);

          // Update local tracking - only update stage if we moved it forward
          const newStageSlug = currentOrder < proposalOrder ? 'proposal-sent' : estimate.existing_stage;
          await client.query(`
            UPDATE crm.crm_opportunities
            SET st_estimate_id = $2,
                ...
                last_synced_at = NOW()
            WHERE crm_id = $1
          `, [estimate.existing_opp_id, estimate.estimate_id, estimate.total || 0, newStageSlug]);

          stats.updated++;
          logger.debug(`Updated estimate: ${title} (stage: ${newStageSlug})`);
