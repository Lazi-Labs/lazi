...
export async function GET() {
  try {
    const response = await fetch(`${PRICING_API_URL}/api/pricing/office-staff?includeInactive=true`, {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${PRICING_API_URL}/api/pricing/office-staff`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    return NextResponse.json({ data: null, error: 'Failed to create office staff' }, { status: 503 });
  }
}
