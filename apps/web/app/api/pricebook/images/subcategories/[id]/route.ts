...
export async function GET(
  ...
  try {
    // Try backend first
    const res = await fetch(`${ST_AUTOMATION_URL}/images/db/subcategories/${id}`, {
    ...
    for (const ext of extensions) {
      const s3Url = `${S3_BUCKET_URL}/${TENANT_ID}/subcategories/${id}${ext}`;
      try {
    ...
    console.error('Subcategory image proxy failed:', error);
    return NextResponse.json(
      { error: 'Failed to fetch subcategory image' },
      { status: 500 }
    );
  }
}
