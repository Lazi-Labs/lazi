...
export async function GET(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${ST_AUTOMATION_URL}/api/pricebook/organization/health`, {
      headers: {
    ...
    if (!res.ok) {
      console.error('Failed to fetch pricebook health:', res.status);
      return NextResponse.json({ success: false, error: 'Failed to fetch health data' }, { status: res.status });
    }

    const result = await res.json();
    return NextResponse.json(result);
  } catch (error) {
    console.error('Failed to fetch pricebook health:', error);
