...
export async function GET(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${ST_AUTOMATION_URL}/api/pricebook/organization/audit-log?${searchParams}`, {
      headers: {
    ...
    if (!res.ok) {
      console.error('Failed to fetch audit-log:', res.status);
      return NextResponse.json({ success: false, error: 'Failed to fetch audit-log' }, { status: res.status });
    }
