...
export async function POST(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${ST_AUTOMATION_URL}/api/pricebook/organization/bulk-update`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      console.error('Failed to bulk update:', res.status, errorText);
      return NextResponse.json({ success: false, error: 'Failed to bulk update' }, { status: res.status });
    }
