...
export async function GET(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${ST_AUTOMATION_URL}/api/pricebook/organization/progress?${searchParams}`, {
      headers: {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ success: false, error: 'Failed to fetch progress' }, { status: res.status });
    }
