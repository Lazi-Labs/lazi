...
export async function POST(
  ...
  try {
    ...
    const res = await fetch(`${ST_AUTOMATION_URL}/api/pricebook/categories/subcategories/${id}/image`, {
      method: 'POST',
      headers: {
...
export async function GET(
  ...
  try {
    const res = await fetch(`${ST_AUTOMATION_URL}/api/pricebook/categories/subcategories/${id}/image/preview`, {
      headers: {
        'x-tenant-id': DEFAULT_TENANT_ID,
      },
    });
    ...
    console.error('Subcategory image preview failed:', error);
    return NextResponse.json(
      { error: 'Failed to get subcategory image preview' },
      { status: 500 }
    );
  }
}
