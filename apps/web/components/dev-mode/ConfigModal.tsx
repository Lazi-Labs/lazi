// =============================================================================
// LAZI AI - Element Configuration Modal (Legacy)
// =============================================================================
...
import { ElementSpec } from './types/devmode.types';

export function ConfigModal() {
  const { state, dispatch, updateSpec } = useDevMode();
  const { isEnabled, currentSpec, selectedElement, currentPage } = state;
  ...
  const handleCancel = () => {
    dispatch({ type: 'SET_SELECTED_ELEMENT', payload: null });
    dispatch({ type: 'SET_CURRENT_SPEC', payload: null });
  };

  const generateWindsurfPrompt = () => {
    const elementText = spec.elementInfo?.textContent?.slice(0, 40) || 'Element';
    const elementType = spec.elementInfo?.tagName || 'Unknown';
    const elementLocation = spec.page || currentPage;
    const elementSelector = spec.elementInfo?.selector || '';

    let prompt = `## Implement: ${elementText} on ${elementLocation}\n\n`;
    prompt += `### Element Details\n`;
    prompt += `- Type: ${elementType}\n`;
    prompt += `- Location: ${elementLocation}\n`;
    prompt += `- Selector: \`${elementSelector}\`\n\n`;
    ...
    switch (spec.behaviorType) {
      case 'api-call':
        ...
        if (spec.uiFeedback?.loadingState) prompt += `3. Show loading spinner during request\n`;
        if (spec.uiFeedback?.successMessage) prompt += `4. On success: Display toast "${spec.uiFeedback.successMessage}"\n`;
        if (spec.uiFeedback?.errorMessage) prompt += `5. On error: Display toast "${spec.uiFeedback.errorMessage}"\n`;
        if (spec.uiFeedback?.refreshAfter) prompt += `6. Refresh table/page data after completion\n`;
        break;
      case 'navigate':
      ...
      case 'sync-job':
        ...
        if (spec.uiFeedback?.loadingState) prompt += `3. Show loading state\n`;
        if (spec.uiFeedback?.refreshAfter) prompt += `4. Refresh data when complete\n`;
        break;
      case 'open-modal':
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ color: '#9ca3af', margin: '4px 0 0', fontSize: '13px' }}>
              {spec.elementInfo?.tagName || 'Element'}: "{spec.elementInfo?.textContent?.slice(0, 40) || 'Unnamed'}"
            </p>
          </div>
          <button
            onClick={handleCancel}
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
                  {[
                    ...
                    { value: 'open-modal', label: '📦 Open Modal', desc: 'Show dialog' },
                    { value: 'custom', label: '✏️ Custom', desc: 'Describe behavior' },
                  ].map(option => (
                    <button
                      key={option.value}
                      onClick={() => updateField('behaviorType', option.value as ElementSpec['behaviorType'])}
                      style={{
                        padding: '12px',
                        backgroundColor: spec.behaviorType === option.value ? '#3b82f6' : '#374151',
                        border: 'none',
                        borderRadius: '8px',
                        cursor: 'pointer',
                        textAlign: 'left',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <input
                  ...
                  onChange={e => updateField('uiFeedback', {
              ...
              <div>
                ...
                <input
                  type="text"
                  value={spec.uiFeedback?.successMessage || ''}
                  onChange={e => updateField('uiFeedback', {
                    loadingState: spec.uiFeedback?.loadingState ?? true,
                    refreshAfter: spec.uiFeedback?.refreshAfter,
              ...
              <div>
                ...
                <input
                  type="text"
                  value={spec.uiFeedback?.errorMessage || ''}
                  onChange={e => updateField('uiFeedback', {
                    loadingState: spec.uiFeedback?.loadingState ?? true,
                    refreshAfter: spec.uiFeedback?.refreshAfter,
                    successMessage: spec.uiFeedback?.successMessage,
