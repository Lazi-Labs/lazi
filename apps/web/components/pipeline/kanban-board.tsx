...
export default function KanbanBoard({
  ...
  const [columns, setColumns] = React.useState<Record<string, Task[]>>(initialColumns || {
    backlog: [
      ...defaultTasks,
      {
        id: "2",
  ...
  const [columnTitles, setColumnTitles] = React.useState<Record<string, string>>(initialColumnTitles || {
    backlog: "Backlog",
  ...
  React.useEffect(() => {
    if (initialColumnTitles) {
      setColumnTitles(initialColumnTitles);
    }
  }, [initialColumnTitles]);
        ...
        <TabsContent value="board">
          <Kanban.Root
            ...
            <Kanban.Board className="flex w-full gap-4 overflow-x-auto pb-4">
              {(initialColumnOrder || Object.keys(filteredColumns)).map((columnValue) => {
                const tasks = filteredColumns[columnValue] || [];
                if (!tasks && !initialColumnOrder) return null;
                return (
                <Kanban.Column
                  key={columnValue}
                  value={columnValue}
